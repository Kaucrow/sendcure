...
pub fn render(app: &App, state: &screens::counter::State, f: &mut Frame) -> Result<()> {
    ...
    let client_ci_scroll = state.inputs.get(0)?.input.visual_scroll(width as usize - client_ci_text.len());
    ...
    let mut sidebar_header_style = Style::default().fg(Color::DarkGray);
    ...
    match state.action_sel {
        Some(0) => {
            ...
            f.set_cursor_position((chunks[0].x
                            + ((state.inputs.get(0)?.input.visual_cursor()).max(client_ci_scroll) - client_ci_scroll) as u16
        ...
        Some(1) => {
            ...
            sidebar_header_style = sidebar_header_style.fg(Color::Green);
        }
        Some(2) => {
            recv_pkg_style = recv_pkg_style.fg(Color::White);
    ...
    let input = Paragraph::new(Text::from(Line::from(vec![
        ...
        Span::styled(state.inputs.get(0)?.input.value(), client_ci_style)
    ])))
    ...
    let sidebar_header = Paragraph::new("Counter")
        .block(sidebar_header_block)
    ...
    // ===============================
    //  Packages operations container
    // ===============================

    let packages_block = Block::default().style(table_style);
