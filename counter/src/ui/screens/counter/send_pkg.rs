...
pub fn render(
    ...
    let desc_width = input_chunks[1].width;
    let desc_style = get_styles(1);
    let desc_text = " Description: ";
    let desc_scroll = tab_state.inputs.get(1)?.input.visual_scroll(desc_width as usize - desc_text.len());
    ...
    let desc_input = Paragraph::new(Text::from(Line::from(vec![
        ...
        Span::styled(tab_state.inputs.get(1)?.input.value(), desc_style.0)
    ])))
    .block(desc_block)
    .scroll((0, desc_scroll as u16));

    f.render_widget(desc_input, input_chunks[1]);

    // ===== Weight input =====

    let weight_width = input_chunks_1[0].width;
    let weight_style = get_styles(2);
    let weight_text = " Weight: ";
    let weight_scroll = tab_state.inputs.get(2)?.input.visual_scroll(weight_width as usize - weight_text.len());
    ...
    let weight_input = Paragraph::new(Text::from(Line::from(vec![
        ...
        Span::styled(tab_state.inputs.get(2)?.input.value(), weight_style.0)
    ])))
    .block(weight_block)
    ...
    let length_style = get_styles(3);
    let length_text = " Length: ";
    let length_scroll = tab_state.inputs.get(3)?.input.visual_scroll(length_width as usize - length_text.len());
    ...
    let length_input = Paragraph::new(Text::from(Line::from(vec![
        ...
        Span::styled(tab_state.inputs.get(3)?.input.value(), length_style.0)
    ])))
    .block(length_block)
    ...
    let width_style = get_styles(4);
    let width_text = " Width: ";
    let width_scroll = tab_state.inputs.get(4)?.input.visual_scroll(width_width as usize - width_text.len());
    ...
    let width_input = Paragraph::new(Text::from(Line::from(vec![
        ...
        Span::styled(tab_state.inputs.get(4)?.input.value(), width_style.0)
    ])))
    .block(width_block)
    ...
    let height_style = get_styles(5);
    let height_text = " Height: ";
    let height_scroll = tab_state.inputs.get(5)?.input.visual_scroll(height_width as usize - height_text.len());
    ...
    let height_input = Paragraph::new(Text::from(Line::from(vec![
        ...
        Span::styled(tab_state.inputs.get(5)?.input.value(), height_style.0)
    ])))
    .block(height_block)
    ...
    if let Some(2) = state.action_sel {
        if let Some(selected_idx) = tab_state.inputs.selected_idx() {
            // 1. Map the active input index to its layout area and prompt length
            let (active_rect, prompt_len) = match selected_idx {
                0 => (input_chunks[0], recipient_text.len()),
                1 => (input_chunks[1], desc_text.len()),
                2 => (input_chunks_1[0], weight_text.len()),
                3 => (input_chunks_1[2], length_text.len()),
                4 => (input_chunks_2[0], width_text.len()),
                5 => (input_chunks_2[2], height_text.len()),
                _ => (Rect::default(), 0),
            };
